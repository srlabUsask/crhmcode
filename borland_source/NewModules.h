...
const long *infDays; // maximum number of days of snowmelt infiltration to frozen soil
...
const long *Volumetric_option;

// declared variables
float *Volumetric;
float *Volumetric_rechr;
float *fallstat_V;
float *Si_V;
float *fallstat;
float *Si;
...
const float *vol_frac_pt1;
const float *vol_frac_pt2;
const float *contrib_frac_pt1;
const float *contrib_frac_pt2;
...
class Classglacier_061718 : public ClassModule {
public:

Classglacier_061718(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl, 1000, " ,snowmeltD"){};
...
Classglacier_061718* klone(string name) const;
};

class ClassSWEslope_061718 : public ClassModule {
public:

ClassSWEslope_061718(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl, 1000, "glacier_All, snowmeltD") {};
...
ClassSWEslope_061718* klone(string name) const;
};
...
float *SRF; // debris-cover melt
float *Xdebris_melt_hrly; // debris-cover melt
float *Xdebris_melt_acc; // debris-cover melt
float *lagT_used; // debris-cover melt, added 2 July 2020
float *lagSW_used; // debris-cover melt, added 2 July 2020
float *den_air; // katabatic parameterization
float *K_kat; // katabatic parameterization
float *Cp_water; // katabatic parameterization
float *Xmelt_int; // katabatic parameterization
float *Xmelt_acc; // katabatic parameterization
float *Qn_kata; // katabatic parameterization
float *Qh_kata; // katabatic parameterization
float *Qe_kata; // katabatic parameterization
float *Qp_kata; // katabatic parameterization
float *Qmelt_kata; // katabatic parameterization
...
const float *debris_h; // debris-cover melt
const float *T_threshold; // debris-cover melt
const long *use_debris; // debris-cover melt
const float *katabatic_lapse_rate; // katabatic parameterization
...
const float *Qsisn_Var; // debris-cover melt
const float *T_rain; // katabatic parameterization
const float *hru_t; // katabatic parameterization
const float *hru_ea; // katabatic parameterization
...
class ClassGen_Mass_Bal : public ClassModule {
public:

ClassGen_Mass_Bal(string Name, String Version = "undefined", CRHM::LMODULE Lvl = CRHM::PROTO) : ClassModule(Name, Version, Lvl) {};
...
float *This_mass_in;
float *This_mass_out;
float *Total_mass_atmos;
float *hru_Total_mass;
float *hru_Total_mass_in;
float *hru_Total_mass_out;
float *hru_Total_mass_atmos;

TStringList *In_Vars_obs; // rain and snow
TStringList *Out_Vars_obs;

TStringList *In_Vars_pbsm;
TStringList *Out_Vars_pbsm;
TStringList *Atmos_Vars_pbsm; // sublimation

TStringList *Out_Vars_PrairieInfiltration;
TStringList *Out_Vars_PrairieInfiltration_Sno;

TStringList *Out_Vars_crack;
TStringList *Out_Vars_crack_Sno;

TStringList *In_Vars_Soil;
TStringList *Out_Vars_Soil;
TStringList *Atmos_Vars_Soil;

TStringList *Atmos_Vars_evap; // evap

TStringList *Out_Vars_Netroute;
TStringList *Out_Vars_Netroute_VU;
...
void Add_mass(TStringList* In_Vars_obs, bool View = NULL);
void Subtract_mass(TStringList* In_Vars_obs, bool View = NULL);
void Transfer_mass(TStringList* In_Vars_obs, bool View = NULL);
void mass_to_Atmosphere(TStringList* In_Vars_obs, bool View = NULL);
void Add_mass_first(TStringList* In_Vars_obs, bool View = NULL);
...
/*    float *Length;
    ...
    const long  *CanopyWindSwitchCanSno;  // Canopy wind model to use for wind induced unloading at 2/3rd canopy height, 0 - no adjustment, 1 - for Prandtl-von Kármán log-linear relationship for above canopy adjustment and Cionco 1965 for within canopy, 2 - for wind profile developed at Fortress sparse canopy".
