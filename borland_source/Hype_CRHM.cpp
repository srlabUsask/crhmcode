...
float fLimit = numeric_limits<float>::max();
...
void ClassWQ_Soil::decl(void) {
  ...
  declstatvar("soil_moist", NHRU, " available moisture content of soil profile to the depth "//
  ...
  declparam("soil_rechr_init", NHRU, "[30.0]", "0.0", "250.0", " Initial value of available water for soil recharge zone (upper part of "// 
  ...
  //  Handle subsurface runoff - does not affect WQ - concentrates
        ...
        if(!inhibit_evap[hh]){
          if(variation == VARIATION_2){ // handle tile drainage
            ...
            if(tile_flow[hh] > 0.0){
              rechr_ssr[hh] = soil_rechr[hh]/soil_rechr_max[hh]*tile_flow[hh]; // take out of recharge layer
              soil_rechr[hh] -= rechr_ssr[hh];
              rechr_ssr_conc_lay[Sub][hh] = conc_top_lay[Sub][hh];

             float remainder = tile_flow[hh] - rechr_ssr[hh];
             if(remainder > soil_moist[hh])
               remainder = soil_moist[hh];
             soil_moist[hh] -= remainder;
             rechr_ssr_conc_lay[Sub][hh] = (rechr_ssr_conc_lay[Sub][hh]*rechr_ssr[hh] + rechr_ssr_conc_lay[Sub][hh]*remainder)/(rechr_ssr[hh] + remainder);
...
ClassGrow_crops_annually* ClassGrow_crops_annually::klone(string name) const{
  ...
  declvar("fert_man_period", NHRU, "current period for fertilizer and manure amount", "()", &fert_man_period);

  declvar("LockOut", NHRU, "prevents changes of fertilizer and manure amounts on multi day periods", "()", &LockOut);

  declvar("SecondDown_fertN", NHRU, "second down", "()", &SecondDown_fertN);

  declvar("SecondDown_fertP", NHRU, "second down", "()", &SecondDown_fertP);

  declvar("SecondDown_manN", NHRU, "second down", "()", &SecondDown_manN);

  declvar("SecondDown_manP", NHRU, "second down", "()", &SecondDown_manP);
  ...
  ObsCntPeriod_fert_man = declreadobs("Fert_period", NHRU, "spreading period for feritilzer and manure", "(d)", &Fert_period, HRU_OBS_Q, true);
...
void ClassGrow_crops_annually::init(void) {
  ...
  if(variation == VARIATION_1 || VARIATION_3){
    ...
    for(hh = 0; hh < nhru; ++hh) {
       ...
       SecondDown_fertN[hh] = 0;
       SecondDown_fertP[hh] = 0;
       SecondDown_manN[hh] = 0;
       SecondDown_manP[hh] = 0;
       fertday1[hh] = 0;
       fertday2[hh] = 0;
  ...
  if(nstep == 1){ // beginning of every day
    for(hh = 0; chkStruct(); ++hh){
      ...
      if(variation == VARIATION_1 || VARIATION_3){
        ...
        if(LockOut[hh] <= 0){
          LockOut[hh] = 1;
          if(ObsCntPeriod_fert_man > 0){ // file open
            if(Fert_period[hh] >= fLimit || !Fert_period[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
              fertperiod[hh] = 1; // set to all one
              LockOut[hh] = 1;
            }
            else{
              if(Fert_period[hh] >=  1.0)
          ...
          fert_man_period[hh] = fertperiod[hh];

          if(ObsCnt_N >= hh){ // file open
            if(Fert_N[hh] >= fLimit || !Fert_N[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
          ...
          if(ObsCnt_P >= hh){ // file open
            if(Fert_P[hh] >= fLimit || !Fert_P[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
          ...
          if(ObsCntMan_N >= hh){ // file open
            if(Man_N[hh] >= fLimit || !Man_N[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
          ...
          if(ObsCntMan_P >= hh){ // file open
            if(Man_P[hh] >= fLimit || !Man_P[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
          ...
          if(ObsCntRes_N >= hh){ // file open
            if(Res_N[hh] >= fLimit || !Res_N[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
          ...
          if(ObsCntRes_P > 0){ // file open
            if(Res_P[hh] >= fLimit || !Res_P[hh]){  // if any value >= fLimit -> NO interval data. fLimit equals 3.402823E+38
