...
//---------------------------------------------------------------------------
TStringList* ClassModule::declGMBparam(string param, CRHM::TDim dimen,
  ...
  if(Global::BuildFlag == CRHM::INIT){
    if(fix && fix->Count > 1){
      ...
      int jj = Global::OurModulesList->IndexOf(S.c_str()); // find module
      if(jj > -1){
        for(long ii = 1; ii < fix->Count; ++ii){
          Newname = fix->Strings[ii];
          if(Global::GroupCntTrk > 0){
            String AA;
            GroupEnding(AA, this->GroupCnt);
            Newname += AA;
          }
          S2 = String(S.c_str()) + " " + Newname;

          ClassVar* V = ClassVarFind(S2.c_str());
          if(V == NULL){
            CRHMException Except("Parameter not in given module: " + Name + param + " in " + S2.c_str(), TERMINATE);
            LogError(Except);
            throw Except;
          }
          fix->Objects[ii] = (TObject *) V;
        }
        return fix;
      }
      else{
            return (TStringList*) 0;
      }
    }
    else{
      CRHMException Except("Module Parameter definition without variable list: " + Name + " -> " + param, TERMINATE);
      LogError(Except);
      throw Except;
    }
  }
        ...
        *value = newVar->values;
        ...
        if(GetUnit > -1){
          *value = newVar->values;
          ...
          CRHMException Except("get variable not found: " + Name + ' ' + name, TERMINATE);
          LogError(Except);
          throw Except;
        }
      }
...
// lag (hours)
  ...
  for(long hh = 0; hh < nhru; hh++){
    ...
    outVar[hh] = 0; // this may have been reset in calling routine. Added 08/09/18
  }
