...
// Calculate sedimentation and resuspension
      ...
      if(qbank > 0.0){
        ...
        if(sedresp > 0.0) { // sedimentation (kg)
          ...
          retention_pool(PPpool, sedPP);              // sedpp may change
          production_pool(tempsed, sedPP);
        }
        else{                 // resuspension (kg)
          resuspPP[0] = - sedresp * tempsed[0];
          retention_pool(tempsed, resuspPP);          // resusppp may change
          production_pool(PPpool, resuspPP);
        }
      }
    }
...
// Denitrification
    ...
    retention_pool(inorganicNpool, denitri_water);
    if(water_r[hh] > 0.0);
...
// Production/mineralisation of organic nitrogen and particulate phosphorus
      if(conductN[hh]){
        ...
        if(minprodN[hh] > 0.0)   // production
         ...
         retention_pool(INpool, minprodN);   // minprodN may be negative
         production_pool(ONpool, minprodN);
      }
      if(conductP[hh]){
        ...
        if(minprodP[0] > 0.0)   // production
         ...
         retention_pool(SRPpool, minprodP);    // minprodP may be negative
         production_pool(PPpool, minprodP);
      }
...
// Calculate production/mineralisation of organic carbon
    ...
    prodC[0] = minprodC[0];
    if(minprodC[0] > 0.0)
      production_pool(OCpool, prodC);
    if(minprodC[0] < 0.0)
      retention_pool(OCpool, minC);
