...
public:
	...
	{
		const double R = 2870;
		return (double)(1E4*Pa / (R*(273.15 + t))*(1.0 - 0.379*(ea / Pa))); //
	}
};
...
public:
	...
	double operator()() const {
		...
		double X = (double)Lvar->get(); // in case there is no array element [n]
		eval.push(X);
		return X;
	}
