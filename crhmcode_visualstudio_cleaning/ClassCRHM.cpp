...
double ClassData::Encode24(int D[]) {
	...
	try {
		if (D[3] == 24) // handle 24:00
			return  Common::EncodeDateTime(D[0], D[1], D[2], 0, D[4]) + 1;
		else
			return  Common::EncodeDateTime(D[0], D[1], D[2], D[3], D[4]);
	}
	catch (...)
	{
		//    string S;
...
bool ClassData::DataReadFile(void) {
	...
	for (Position = 0; Position < Lines; ++Position) 
	{
		...
		//Data[11][Position] = Common::estar(Data[0][Position])* Data[1][Position] / 100.0; // Manishankar look at this.
	...
	Common::writefile("Datareadfile: "+std::to_string(timediff));
...
bool ClassData::DataReadFile(void) {
 ...
 string Header, Var, Comment;
	...
	if (!Simulation) {
		...
		do {
			...
			if (Dt1 > 3000.0)
				...
				DecimalTime = false;
				DataFile.seekg((int)Here - 1);  // go back to beginning of data
				for (int ii = 0; ii < 5; ii++)
					DataFile >> D[ii];
				Position = HdrLen;
				Dt1 = Encode24(D);
			...
			if (Cols < DataCnt) {
				CRHMException Except("Observation file header Error " + DataFileName, ERR);
				//Application->MessageBox(Except.Message.c_str(),
					//string("Header of file defines more observations than data columns (" + std::to_string(Cols) + ")").c_str(), MB_OK);
				LogError(Except);
				DataFile.close();
				return false;
			}
				...
				CRHMException Except("Observation file header Warning - Column count > Data count " + DataFileName, WARNING);
				//Application->MessageBox(Except.Message.c_str(),
					//string("Header of file defines fewer observations than data columns (" + std::to_string(Cols) + ")").c_str(), MB_OK);
				LogError(Except);
			}
		...
		if (!OneLine) {
			...
			DataFile.seekg(0, ios_base::end);
			ifstream::off_type Back = (int)DataFile.tellg() - 3;
			char c;
	...
	HCURSOR OriginalCursor = GetCursor(); //getting the current cursor.
		
	hCurs1 = LoadCursor(NULL, IDC_WAIT); // Create a standard hourglass cursor.
	SetCursor(hCurs1);
	...
	try {
		...
		for (Position = 0; Position < Lines; ++Position) {
			if (Position % 100 == 0) {
				if (!Simulation)
					Update_Main_Status("Observation Line " + to_string(Position + 1));
				else
					Update_Main_Status("Simulation Line " + to_string(Position + 1));
			...
			if (!Simulation) {
				...
				if (Position != 0 && Times[Position] <= Times[Position - 1]) {
				 string S;
					printf("Earlier Date at line: %u+, %5u %3u %3u %3u %3u in observation file ", Position, D[0], D[1], D[2], D[3], D[4]);
				...
				for (int ii = 0; ii < DataCnt; ++ii) {
					...
					if (DataFile.fail()) { // handles faulty data
					 string S = string("Faulty data at line ") + to_string(HdrLen + Position + 1)
							+ string(" in observation file ");
						CRHMException Except(S.c_str() + DataFileName, ERR);
						//Application->MessageBox(Except.Message.c_str(),
							//"Project observation file", MB_OK);
				...
				if (Position != 0 && Times[Position] >= Times[Position - 1] + DeltaH)
					if (!SparseFlag) {
						...
						if (FirstFile) {
						 string S;
							if (DecimalTime)
								printf("Discontinuous Date: \"%5f\" at line: %u", Times[Position], Position + HdrLen + 1);
							else
								printf("Discontinuous Date: \"%5u %3u %3u %3u %3u\" at line: %u", D[0], D[1], D[2], D[3], D[4], HdrLen + Position + 1);
...
long julian(char *when) {
	...
	if (!strcmp(when, "start")) {
		Common::DecodeDateTime(Global::DTstart, &Y, &M, &D, &H, &Min);
		DtBegin = Common::EncodeDateTime((Word)Y - 1, (Word)12, (Word)31, 0, 0);
		return  INT(Global::DTstart - DtBegin);  // -0.999/Global::Freq
	}
	else if (!strcmp(when, "end")) {
		Common::DecodeDateTime(Global::DTend, &Y, &M, &D, &H, &Min);
		DtBegin = Common::EncodeDateTime((Word)Y - 1, (Word)12, (Word)31, 0, 0);
...
long julian(double DT) {
	...
	DtBegin = Common::EncodeDateTime(Y - 1, 12, 31, 0, 0);
...
long MonthDaysCnt(double DT) {
	...
	DTNext = Common::EncodeDateTime((Word)Y, (Word)M, (Word)D, 0, 0);
...
void dattim(char *type, long *itime) { // dimension itime[6]
	...
	if (type == "start") {
		Common::DecodeDateTime(Global::DTstart, &Year, &Month, &Day, &H, &Min);
		//DecodeTime(Global::DTstart, &H, &Min, &Sec);
	}
	else if (type == "end") {
		Common::DecodeDateTime(Global::DTend, &Year, &Month, &Day, &H, &Min);
		//DecodeTime(Global::DTend, &Hour, &Min, &Sec);
	}
	else {
		Common::DecodeDateTime(Global::DTnow, &Year, &Month, &Day, &H, &Min);
...
void dattim(double DT, long *itime) { // dimension itime[6]
	...
	Common::DecodeDateTime(DT, &Year, &Month, &Day, &H, &Min);
...
void Classfilter::readargs() {
	...
	long ee = Pos(args, ')');
	string Comment = (args.substr(ee + 1, args.length())).c_str();
	//  Comment = Comment.Trim();
	if (Comment.empty())
		Comment = "filter";

	args = args.substr(1, ee - 1);
	while (ee = Pos(args, ','), ee > 0) args[ee] = ' ';
...
void Classmacro::addfilter (string Line) {
	...
	string S = Line.substr(1, Line.length());

	if (Line.substr(2, 1) == "$") return; // handle comment
	...
	long ee;
	for (ee = 1; S[ee] != '('; ++ee)
		if (IsDelimiter(S," \t", ee)) break;

	string ToVar = "";
	if (bb > ee)    // filter returns value
		ToVar = S.substr(1, ee - 1);
	else
		ee = 1;      // only "sim" ForceInterval and TimeShift at present

	S = Trim(S.substr(ee, Line.length()));

	ee = Pos(S,'(');
 string Filter = S.substr(1, ee - 1);
 string FilterStuff = Trim(S.substr(ee + 1, Line.length()));
...
long julian_month(long Year, long julian) {
	...
	double DT = Common::EncodeDateTime((Word)Year - 1, (Word)12, (Word)31, 0, 0);

	DT += (double)julian;
	Common::DecodeDateTime(DT, &Y, &Month, &Day, &H, &Min);
	return Month;
}
