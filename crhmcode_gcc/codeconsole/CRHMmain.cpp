...
try {
	...
	do {
		...
		if (S == "AKAs:") {
			...
			//Common::Message("test 2", S);
			MacroLoad();
			//Common::Message("test 3", S);
		}
		else if (S == "Observations:") {
			getline(DataFile, S);

			//Common::Message ("observation name line = ",S);
			...
			while (S[0] != '#') {
				...
				if (FindFileName(SS)) {
					...
					Common::Message(SS, "Cannot find observation file");
				}
...
void CRHMmain::MacroLoad(void)
{
	...
	if (Global::MacroModulesList->Count > 0) {
		...
		while (Macro < Global::MacroModulesList->Count) {
			...
			//Common::Message ("test 8-1", "test 8-1");

			AdminMacro.AddModule(Custom);

			//Common::Message ("test 9", "test 9");
			...
			while (S = Common::trim(Global::MacroModulesList->Strings[Macro]), SS = S.substr(0, 3),
				...
				++Macro;
			//Common::Message ("test 10", "test 10");
...
bool  CRHMmain::FileExistsSp(string FileName) {
	...
	//Common::Message ("here 1: ", std::to_string(DataFile.good ()));


	/*
	if (DataFile) {
		Common::Message ("here 1: ", FileName.c_str());
		DataFile.close();
		return true;
	}
	else	{
		Common::Message ("here 2:", FileName.c_str());
		return false;
	}*/
}
//---------------------------------------------------------------------------
...
void  CRHMmain::RunClick(void) {
	...
	//Common::Message("test 1", "test 1");

	double DTstartR = Picker1;
	double DTendR = Picker2;

	//Common::Message ("test DTstartR", std::to_string(DTstartR));
	//Common::Message ("test DTendR", std::to_string(DTendR));
	//Common::Message ("test Global::DTstart", std::to_string(Global::DTstart));
	...
	if (Global::DTmin < 0) {Global::DTmin = 0;}
    ...
    //Common::Message("test 3", "test 3");

	try {
	    //Common::Message("test 3", std::to_string(Global::OurModulesList->Count));
		for (Modii = 0; Modii < Global::OurModulesList->Count; Modii++)
        {
            //Common::Message("test 3", std::to_string(Modii));
            ((ClassModule*)(Global::OurModulesList->Objects[Modii]))->initbase();
            //Common::Message("test 3", "again");
        }
	}
	    ...
	    //Common::Message("test 3", "test 3-11");
		...
		//Common::Message("test 3", "test 3-111");
		LogMessageX(E.what());
		GoodRun = false;
	}

	catch (...) {
	    //Common::Message("test 3", "test 3-1111");
		LogMessageX("Unknown error");
		GoodRun = false;
	}

	//Common::Message("test 3", "test 3-2");
	...
	if (GoodRun) {
        //Common::Message("test 5-1", "test 5-1");
		if (!OpenStateFlag) {
	...
	//Common::Message("test 5-2", "test 5-2");
...
void  CRHMmain:: AllRprt(void)
{
	...
	for (int nn = 0; nn < cdSeries[0]->Count(); ++nn) {
		...
		//Sx = FloatToStrF(cdSeries[0]->XValue(nn), ffGeneral, 10, 0); //commented by Manishankar.
		Sx = StandardConverterUtility::GetDateTimeInStringForOutput(cdSeries[0]->XValue(nn)); //added by Manishankar to show the date in the format: <year month day hour minute>
...
void CRHMmain:: RprtHeader(TStringList *LogList, int LocalCnt)
{
	...
	for (int vv = 0; vv < LocalCnt; ++vv) {
		...
		Sx += string(" 1 ");
		//Sx += thisVar->units; //Manishankar did this. GCC is showing segmentation fault here. thisVar is null.
		LogList->Add(Sx);
	}
