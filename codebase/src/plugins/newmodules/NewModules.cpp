...
void Classnetall::decl(void) {
  ...
  declgetvar("*", "Tday", "(Â°C)",   &NotUsed, &tday_intvls); // from obs
...
void Classebsm::decl(void) {
  ...
  declgetvar("*", "hru_tmean", "(Â°C)",  &tmean);
  declgetvar("*", "hru_tmax", "(Â°C)",   &tmax);
  declgetvar("*", "hru_tmin", "(Â°C)",   &tmin);
  ...
  decldiagparam("tfactor", NHRU, "[0.0]", "0.0","10.0", "degree day melt factor", "(mm/d*Â°C)", &tfactor);
...
void ClassTs::decl(void) {
  ...
  declvar("hru_Ts", NHRU, "snow surface temperature", "(Â°C)", &hru_Ts);
...
// parameters:
  ...
  declparam("unload_t", NHRU, "[1.0]", "-10.0", "20.0", "if ice-bulb temp >= t : canopy snow is unloaded as snow", "(Â°C)", &unload_t);

  declparam("unload_t_water", NHRU, "[4.0]", "-10.0", "20.0", "if ice-bulb temp >= t: canopy snow is unloaded as water", "(Â°C)", &unload_t_water);
...
void ClassPSPnew::decl(void) {
  ...
  declvar("TCanSnow", NHRU, "snow Canopy temperature", "(Â°C)", &TCanSnow);

  declvar("Tbiomass", NHRU, "biomass temperature", "(Â°C)", &Tbiomass);
  ...
  declgetvar("obs",    "hru_t",  "(Â°C)",   &TAref);
  declgetvar("obs",    "hru_rh", "(%)",  &RHref);
  declgetvar("obs",    "hru_p",  "(mm/int)",   &hru_p);
  declgetvar("*",      "SolAng", "(r)",  &SolarAng);

  declreadobs("TsnowG", NHRU, "snow temperature", "(Â°C)", &TsnowG, HRU_OBS_misc);
...
void Classbrushintcp::decl(void) {
  ...
  declparam("Tunload", NHRU, "0.0", "-5.0", "5.0", "Canopy unload at daily mean temperature", "(Â°C)", &Tunload);
  ...
  declgetvar("obs",  "hru_t", "(Â°C)", &hru_t);
  declgetvar("obs",  "hru_tmean", ", (Â°C)", &hru_tmean);
...
void ClassHMSA::decl(void) {
  ...
  Description = "'Estimation of frost table (Masaki Hayashi 2007).' \
                 'ebsm using variable hru_t (Â°C),' \
                 'ebsm using observation Tsurf_obs (Â°C).'";
...
// Variables:
  ...
  declvar("Lamda_b", NHRU, "Thermal conductivity", "()", &Lamda_b); // (W/m.Â°C)
  ...
  declvar("Last_Tsurf", NHRU, "last surface temperature", "(Â°C)", &Last_Tsurf);
...
// driving variable:
  ...
  declreadobs("Tsurf_obs", NHRU, "soil surface temperature observation", "(Â°C)", &Tsurf_obs, HRU_OBS_misc);
...
void ClassSoil::decl(void) {
  ...
  declparam("stream_runoff", NHRU, "[0.0]", "0.0", "1.0", "fraction of stream returned to runoff.", "()", &stream_runoff);
  ...
  declvar("weir_outflow", NHRU, "weir_outflow.", "(m^3/int)", &weir_outflow);

  declvar("weir_outflow_m3s", NHRU, "weir_outflow.", "(m^3/s)", &weir_outflow_m3s);

  declvar("weir_water_A", NHRU, "surface area of weir pond.", "(m^2)", &weir_water_A);

  declvar("weir_water_V", NHRU, "volume of weir pond.", "(m^3)", &weir_water_V);

  declvar("weir_water_h", NHRU, "depth of weir pond.", "(m)", &weir_water_h);

  declvar("weir_evap", NHRU, "Depth of water evaporating from weir pond.", "(mm/int)", &weir_evap);

  declvar("weir_crest_volume", NHRU, "volume of pond at weir crest.", "(m^3)", &weir_crest_volume);

  declstatdiag("cum_weir_over_m3", NHRU, "Cumulative weir HRU overflow.", "(m^3)", &cum_weir_over_m3);


  declvar("stream_to_Sd", NHRU, "amount of soil runoff to stream.", "(mm/int)", &stream_to_Sd);

  declparam("stream_Sd", NHRU, "[0.0]", "0.0", "1.0", "fraction of stream returned to Sd.", "()", &stream_Sd);

  declparam("weir_crest", NHRU, "[10]", "0.0", "200.0", "weir crest height.", "()", &weir_crest);

  declparam("weir_coefficient", NHRU, "[1.705]", "0.01", "2.0", "weir_coefficient.", "()", &weir_coefficient);

  declparam("weir_mult", NHRU, "[1.0]", "0.01", "2.0", "weir_multiplier)", "()", &weir_mult);

  declparam("weir_b", NHRU, "[0.0]", "0.0", "1000.0", "breadth of weir. . Zero = no weir", "(m)", &weir_b);

  declparam("lake_mult", NHRU, "[1.0]", "1.0", "1.0e6", "lake rating curve multiplier.", "()", &lake_mult);

  declparam("lake_power", NHRU, "[1.0]", "0.1", "5.0", "lake rating curve power.", "()", &lake_power);

  declparam("lake_p", NHRU, "[1.72]", "1.0", "10.0", "lake shape factor.", "()", &lake_p);

  declparam("lake_S", NHRU, "[1.72]", "1000.0", "200000.0", "lake shape factor.", "()", &lake_S);


  variation_set = VARIATION_1 + VARIATION_2;

  declvar("soil_runoff_stream", NHRU, "amount of soil runoff to stream.", "(m^3)", &soil_runoff_stream);

  declparam("runoff_stream", NHRU, "[0.0]", "0.0", "1.0", "fraction of runoff diverted to stream.", "()", &runoff_stream);

  declvar("stream", NHRU, "Portion of runoff water retained in HRU as streamflow.", "(mm*km^2/int)", &stream);

  declvar("cum_stream", NHRU, "cumulative portion of runoff water retained in HRU as streamflow.", "(mm*km^2/int)", &cum_stream);

  declvar("stream_Grp_in", NHRU, "streamflow from other group (sub-basin.", "(mm*km^2/int)", &stream_Grp_in);

  declvar("stream_Grp_loss", BASIN, "amount of stream flow out of this sub-basin.", "(m^3)", &stream_Grp_loss);

  declvar("cum_stream_Grp_loss", BASIN, "cumulative amount of stream flow out of this sub-basin.", "(m^3)", &cum_stream_Grp_loss);
  ...
  declvar("cum_Sd_evap", NHRU, "cumulative Sd evap over HRU.", "(mm)", &cum_Sd_evap);

  declvar("soil_gw", NHRU, "Portion of excess soil water from a HRU that enters groundwater reservoirs.", "(mm/int)", &soil_gw);

  declvar("Sd_gw", NHRU, "Portion of excess Sd from a HRU that enters groundwater reservoirs.", "(mm/int)", &Sd_gw);
  ...
  decldiagparam("Wetlands_scaling_factor", NHRU, "[-1.0]", "-1.0", "1.0","Changes the normal evap ratio 'Sd/(Sd + soil_moist'. -1 -> normal to 0.0 to 1.0 -> programmable.", "()", &Wetlands_scaling_factor); // temporary
...
void Classevap_Resist::decl(void) {
  ...
  declreadobs("Ts", NHRU, "surface temperature", "(Â°C)", &Ts, HRU_OBS_misc, true);
...
void ClassIceBulb::decl(void) {
  ...
  declvar("hru_icebulb", NHRU, "Estimates ice bulb temperature", "(Â°C)", &hru_icebulb);
...
void Classlake::decl(void) {
  ...
  declvar("hru_t_Mmean", NHRU, "HRU temperature", "(Â°C)", &hru_t_Mmean);

  declvar("hru_rh_Mmean", NHRU, "HRU rh", "()", &hru_rh_Mmean);

  declvar("hru_u_Mmean", NHRU, "HRU wind speed", "(m/s)", &hru_u_Mmean);

  decllocal("hru_t_acc", NHRU, "HRU t accumulated", "(Â°C)", &hru_t_acc);
...
void ClassXG::decl(void) {
  ...
  declstatvar("Bfr", NHRU, "freeze degree days", "(ÂºC*d)", &Bfr);

  declstatvar("Bth", NHRU, "thaw degree days", "(ÂºC*d)", &Bth);
  ...
  decllocal("t_trend", NHRU, "temperature long term trend", "(Â°C)", &t_trend);

  decllocal("check_t", NHRU, "check temperature source", "(Â°C)", &check_t);
  ...
  decldiagparam("Trigthrhld", NHRU, "[100.0]", "50.0", "1000.0", "Trigger reference level in degree days", "(ÂºC*d)", &Trigthrhld);
...
void ClassFlowInSnow::decl(void) {
  ...
  declparam("t_inhibit_flow", NHRU, "[-2.0]", "-15", "5", "threshold temperature of inhibit flow in snow; i.e. inhibit when lower snowpack temperature is below this threshold temperature", "(Â°C)", &t_inhibit_flow);
...
void Classtsurface::decl(void) {
  ...
  declvar("hru_tsf", NHRU, "surface/skin temperature", "(Â°C)", &hru_tsf);

  declvar("hru_tsf_D", NHRU, "mean daily surface/skin temperature", "(Â°C)", &hru_tsf_D);

  decldiag("hru_tsf_acc", NHRU, "mean daily surface/skin temperature", "(Â°C)", &hru_tsf_acc);
  ...
  decllocal("hru_T_s_D", NHRU, "mean daily surface/skin temperature", "(Â°C)", &hru_T_s_D);

  decllocal("hru_T_s_acc", NHRU, "sum interval air temperature from previous day", "(Â°C)", &hru_T_s_acc);

  declgetvar("*", "z_s", "(m)", &z_s);

  declgetvar("*", "rho", "(kg/m^3)", &rho);

  declgetvar("*", "T_s", "( Â°C)", &T_s);
  ...
  decllocal("hru_t_D", NHRU, "mean daily snowpack temperature from previous day", "(Â°C)", &hru_t_D);

  decllocal("hru_t_acc", NHRU, "sum interval air temperature from previous day", "(Â°C)", &hru_t_acc);
...
void Classqmelt::decl(void) {
  ...
  declparam("TIa0Mn", NHRU, "[0.0]", "-10", "10", "Qm: degree day/daily mean air temperature", "(Â°C)", &TIa0Mn);

  declparam("TIa1Mn", NHRU, "[0.8844, 0.0]", "0", "10", "Qm: degree day/daily mean air temperature", "(MJ/d*Â°C)", &TIa1Mn);

  declparam("TIs0Mn", NHRU, "[0.0]", "-10", "10", "Qg: degree day/daily mean skin temperature", "(Â°C)", &TIs0Mn);

  declparam("TIs1Mn", NHRU, "[0.0, 0.2]", "0", "10", "Qg: degree day/daily mean skin temperature", "(MJ/d*Â°C)", &TIs1Mn);


  declreadobs("ta", NOBS, "air temperature", "(Â°C)", &t);

  declreadobs("ts", NOBS, "skin temperature", "(Â°C)", &ts);
...
void Classquinton::decl(void) {
  ...
  decllocal("tlayer", NLAY, "layer temperature.", "(Â°C)", &tlayer, &tlayer_lay);
  ...
  declparam("tinit", NLAY, "[-8.0] , [-8.0], [-8.0] ,[-8.0]", "-40.0", "10.0", "initial layer temperature.", "(Â°C)", &tinit, &tinit_lay);
...
void ClassREWroute_stream::init(void) {
  ...
  if(variation == VARIATION_ORG){
    ...
    for(hh = 0; hh < nhru; ++hh){
      ...
      if(WS_stream_Ktravel_var[hh] < (Global::Interval/(2.0*(1.0-WS_stream_route_X_M[hh])))){ //inflowDelay->c0[hh] < 0.0
...
void ClassREWroute_stream::run(void) {
  ...
  for(long jj = 0; jj < instreamflowCnt; ++jj){ // HRUs not in sequence
    ...
    if(to_GRP > 0 && WS_stream_wherefrom_HRU[hh] > 0){ // otherwise do nothing
      ...
      if(variation == VARIATION_ORG)
        instreamflowDelay->DoMuskingum(hh); // need to update for later HRUs
      else if(variation == VARIATION_1)
        Clark_instreamflowDelay->DoClark(hh); // need to update for later HRUs

      if(outstreamflow[hh] > 0.00001){
        cumoutstreamflow[hh] += outstreamflow[hh];

        if(WS_stream_whereto_HRU[hh] > 0 && WS_stream_whereto_HRU[hh] < rew[to_GRP]){ // put in other sub-basin HRU
            instreamflow_Grp_out_All[to_GRP][WS_stream_whereto_HRU[hh]-1] += outstreamflow[hh];
        }
        else if(WS_stream_whereto_HRU[hh] == 0) { // limited use?
          streamflow[0] += outstreamflow[hh]; // (already m^3)
...
void Classglacier::decl(void){
  ...
  declparam("TKMA", NHRU, "[30]", "-50", "50", "Mean annual temperature of glacier", "(ÂºC)", &TKMA);
...
void Classglacier_debris::decl(void){
  ...
  declvar("Gn_ebsm", NHRU, "net radiation", "(MJ/d)", &Qn_ebsm);

  declvar("Gh_ebsm", NHRU, "sensible heat", "(MJ/d)", &Qh_ebsm);

  declvar("Ge_ebsm", NHRU, "latent heat", "(MJ/d)", &Qe_ebsm);

  declvar("Gp_ebsm", NHRU, "input from rainfall", "(MJ/d)", &Qp_ebsm);
...
// debris 
  ...
  declvar("lagT", NHRU, "lagged temperature.", "(Â°C)", &lagT);
  ...
  declparam("T_threshold", NHRU, "[1.0]", "-30", "30", "threshold temperature for melt to occur.", "(Â°C)", &T_threshold);

  declreadobs("lagT1", NHRU, "lagged temperature 1", "(Â°C)", &lagT1, HRU_OBS_Q, true);

  declreadobs("lagT2", NHRU, "lagged temperature 2", "(Â°C)", &lagT2, HRU_OBS_Q, true);

  declreadobs("lagT3", NHRU, "lagged temperature 3", "(Â°C)", &lagT3, HRU_OBS_Q, true);

  declreadobs("lagT4", NHRU, "lagged temperature 4", "(Â°C)", &lagT4, HRU_OBS_Q, true);
