...
InstrumentLogger::instance()->log_instrument_log("<ClassIceBulb::decl(void)@@@ClassIceBulb.cpp>");
    ...
    decldiagparam ("tmaxI_allrain", TDim :: NHRU, "[1.0]", "-10", "10", "precip all rain if HRU ice bulb temperature above or equal to this value", "(°C)", & tmaxI_allrain);
    decldiagparam ("tmaxI_allsnow", TDim :: NHRU, "[-1.0]", "-10", "10", "precip all snow if HRUice bulb  temperature below this value", "(°C)", & tmaxI_allsnow);
InstrumentLogger::instance()->log_instrument_log("</ClassIceBulb::decl(void)@@@ClassIceBulb.cpp>");
}
