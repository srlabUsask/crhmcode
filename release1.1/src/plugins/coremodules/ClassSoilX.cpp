...
{
InstrumentLogger::instance()->log_instrument_log("<ClassSoilX::klone(string name) const@@@ClassSoilX.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassSoilX::decl(void)@@@ClassSoilX.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassSoilX::init(void)@@@ClassSoilX.cpp>");
    ...
    for (hh = 0; hh < nhru; ++ hh) {
        ...
        thaw_layers_lay [0] [hh] = 1.0;
        thaw_layers_lay [1] [hh] = 1.0;
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassSoilX::run(void)@@@ClassSoilX.cpp>");
    ...
    for (hh = 0; chkStruct (); ++ hh) {
        ...
        if (nstep == 0)
        {
            ...
            if (soil_moist_max [hh] > 0.0 && ! NO_Freeze [hh] && Zd_front_array [0] [hh] > 0.0)
            {
                ...
                while (ll < depths_size && Zdt [hh] > 0.0) {
                    ...
                    {
                        if (Zdt [hh] >= layer_start && Zdt [hh] < layer_end)
                        {
        ...
        if (soil_moist_max [hh] > 0.0)
        {
            ...
            soil_rechr [hh] = soil_rechr [hh] + possible;
            if (soil_rechr [hh] > soil_rechr_max [hh])
            {
            ...
            if (! inhibit_evap [hh])
            {
                rechr_ssr [hh] = soil_rechr [hh] / soil_rechr_max [hh] * rechr_ssr_K [hh] / Global :: Freq * thaw_layers_lay [0] [hh];
            ...
            double s2gw_k = soil_gw_K [hh] / Global :: Freq * thaw_layers_lay [1] [hh];
        ...
        {
            double s2ssr_k = lower_ssr_K [hh] / Global :: Freq * thaw_layers_lay [1] [hh];
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassSoilX::finish(bool good)@@@ClassSoilX.cpp>");
