...
{
InstrumentLogger::instance()->log_instrument_log("<ClassPSPnew::klone(string name) const@@@ClassPSPnew.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassPSPnew::decl(void)@@@ClassPSPnew.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassPSPnew::init(void)@@@ClassPSPnew.cpp>");
...
double SatVP (double Temp)
{
InstrumentLogger::instance()->log_instrument_log("<SatVP(double Temp)@@@ClassPSPnew.cpp>");
    if (Temp > 0.0)
    {
...
InstrumentLogger::instance()->log_instrument_log("</SatVP(double Temp)@@@ClassPSPnew.cpp>");
        return 611.0 * exp (21.88 * Temp / (Temp + 265.5));
    }
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassPSPnew::run(void)@@@ClassPSPnew.cpp>");
    const double GapFrac = 0.16;
    const double UpperGF = 0.58;
    const double Radius = 0.0005;
    const double KinVisc = 1.88E-5;
    const double R = 8313.0;
    const double M = 18.01;
    const double RhoI = 900.0;
    const double k1 = 0.0114;
    const double Fract = 0.4;
    const double SnowAlb = 0.8;
    const double CanAlb = 0.2;
    const double KARMAN = 0.4;
    const double g = 9.8;
    const double SBC = 5.67E-8;
    const double SpHtAir = 1013.0;
    const double SpHtIce = 2090.0;
    const double SpHtCan = 2700.0;
    const double Hs = 2838000.0;
    long TItNum, RHItNum, TItNum2;
    bool Tup, RHup, Tup2;
    double I1, Cp, wtsubl, Unld;
    double StepT, StepRH, StepT2;
    double DblRHcan, DblTbarCan, DblTCanSnow;
    double RhoS, Lstar;
    double SVDensC, SVDensS, LambdaT, CanVent, A, Nr, NuSh, D, Vs, Vhr, Ce, Vi;
    double QTrans50, QTrUp50, CanSnowFrac;
    double RhoAir, SVDensA, VPref, Ustar, Uh, Ra, Ri, Stabil, Qe, Qh, dUdt;
    static long N;
    ...
    for (long hh = 0; hh < nhru; hh ++) {
        ...
        A = 2.4338 + 3.45753 * exp (- Uh);
        CanVent = Uh * exp (A * ((Zcan [hh] - 10) / (Ht [hh] - 10) - 1.0));
        ...
        do {
            ...
            Ri = g / ((TAref [hh] + DblTbarCan) / 2.0 + 273.15) * (TAref [hh] - DblTbarCan) / (Zref [hh] - Ht [hh]) / (sqr ((hru_u [hh] - Uh) / (Zref [hh] - Ht [hh])));
            if (Ri < 0.0)
            {
            ...
            do {
                ...
                do {
                    ...
                    {
                        DblTCanSnow += StepT2;
                        Tbiomass [hh] = DblTbarCan + ((DblTbarCan - DblTCanSnow) * Load [hh] * SpHtIce) / (Biomass [hh] * SpHtCan);
                    }

                    double QlwOut = SBC * (GapFrac * pow (TsnowG [0] + 273.15, 4.0) + (1.0 - GapFrac) * ((1.0 - CanSnowFrac) * pow (Tbiomass [hh] + 273.15, 4.0) + CanSnowFrac * pow (DblTCanSnow + 273.15, 4.0)));
                    ...
                    double QlwDnStar = M_PI * Radius * Radius * (QlwIn * UpperGF + SBC * (1.0 - UpperGF) * ((1.0 - CanSnowFrac) * pow (Tbiomass [hh] + 273.15, 4.0) + (CanSnowFrac * pow (DblTCanSnow + 273.15, 4.0))));
            ...
            dUdt = ((Tbiomass [hh] - T0biomass [hh]) * Biomass [hh] * SpHtCan + (DblTCanSnow - T0CanSnow [hh]) * Load [hh] * SpHtIce) / (Global :: Interval * 24 * 3600.0);
            if (Qh + Qe - dUdt < 0.0)
            {
                if (TItNum == 1)
                {
        ...
        T0biomass [hh] = Tbiomass [hh];
        wtsubl = - Qsubl [hh] * Global :: Interval * 24 * 3600.0 / Hs;
        if (wtsubl > Load [hh])
        {
            Qsubl [hh] = - Load [hh] / (Global :: Interval * 24 * 3600.0 / Hs);
            wtsubl = Load [hh];
            Load [hh] = 0.0;
        }
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassPSPnew::finish(bool good)@@@ClassPSPnew.cpp>");
    delete [] T0CanSnow;
    T0CanSnow = NULL;
    delete [] T0biomass;
    T0biomass = NULL;
