...
InstrumentLogger::instance()->log_instrument_log("</ClassWQ_SoilBGC::nutrient_inputs()@@@WQ_Soil_NPCcycling.cpp>");
    ...
    try {
        ...
        string S = string ("Error ") + " at " + StandardConverterUtility :: GetDateTimeInString (Global :: DTnow) + " (" + to_string (getstep ()) + ") in '" + Global :: OurModulesList -> Strings [Global :: CurrentModuleRun] + "'" + " (" + FloatToStrF (Global :: DTnow, TFloatFormat :: ffGeneral, 10, 6) + ") hh = " + FloatToStrF (hh, TFloatFormat :: ffGeneral, 6, 0);
