...
void ClassWQ_REWroute::decl(void) {
  ...
  inflowConcCnt = declgrpvar("WS_ALL_inflow_conc", "basinflow_conc", "query variable = 'basinflow_conc'", "(g/l)", &rew_conc, &inflow_conc_All);

  declvar("WS_inflow", TDim::NHRU, "inflow from each RB", "(m^3/int)", &inflow);
  declvar("WS_inflow_mWQ", TDim::NDEFN, "Mass-flux: inflow from each RB", "(kg/int)", &inflow_mWQ, &inflow_mWQ_lay, numsubstances);
  ...
  declvar("WS_outflow_mWQ", TDim::NDEFN, "Mass-flux: outflow of each RB", "(kg/int)", &outflow_mWQ, &outflow_mWQ_lay, numsubstances);
  declvar("WS_outflow_cWQ", TDim::NDEFN, "Mass-flux: outflow of each RB", "(g/l)",    &outflow_cWQ, &outflow_cWQ_lay, numsubstances);
...
void ClassWQ_REWroute::init(void) {
  ...
  if(variation == VARIATION_ORG){
    ...
    int invalidHru = inflowDelay->invalidHru();
    if (invalidHru >= 0) {
        hh = invalidHru;
        cout << "REWroute: Options for correcting Muskingum:" << endl;
//                K > Global::Interval/(2.0*(1.0 - X_M[hh]))
        double Vavg = (1.0/WS_route_n[hh])*pow(WS_route_R[hh], 2.0f/3.0f)*pow(WS_route_S0[hh], 0.5f);
        std::cout << "Increase WS_route_L[" << hh << "] from " << WS_route_L[hh] << " to above " << ( (Vw[WS_Channel_shp[hh]] * Vavg) * 86400.0 * Global::Interval/(2.0*(1.0 - WS_route_X_M[hh])) ) << endl;
...
void ClassWQ_REWroute::run(void) {
  ...
  for(long jj = 0; jj < inflowCnt; ++jj) { // HRUs not in sequence
    ...
    if(rew[hh]) {
      ...
      for (long Sub = 0; Sub < numsubstances; ++Sub) {
        inflow_mWQ_lay[Sub][hh] = inflow_conc_All[hh][Sub] * inflow[hh];
      }
    } else {
