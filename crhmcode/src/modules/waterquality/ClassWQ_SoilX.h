...
#ifndef CLASSWQ_SOILX
#define CLASSWQ_SOILX

#include "WQ_CRHM.h"

class ClassWQ_SoilX : public ClassModule {
public:

	ClassWQ_SoilX(string Name, string Version = "undefined", LMODULE Lvl = LMODULE::PROTO) : ClassModule(Name, Version, Lvl) {};
...
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	ClassWQ_SoilX* klone(string name) const;
};
