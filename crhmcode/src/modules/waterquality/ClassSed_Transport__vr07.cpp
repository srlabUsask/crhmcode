...
double ClassSed_Transport__vr07::calc_critical_shear_stress_nodim_VANRIJN(double diam_nodim) {
...
double ClassSed_Transport__vr07::calc_vanrijn_flowdepth_from_manning(double flow_rate) {
//  global mannings_n
...
double ClassSed_Transport__vr07::calc_vanrijn_flowvel_from_flowdepth(double flow_rate, double h) {
  return flow_rate / (h*h);
}

double ClassSed_Transport__vr07::calc_bed_shear_stress(double stream_depth) {
  return rho_w * g * stream_depth * tan(channel_slope[hh]);
}

double ClassSed_Transport__vr07::calc_nodim_shear_stress( double tau_b ) {
  return tau_b / ( rho_w * (s-1) * g * diam50[hh] );
}

double ClassSed_Transport__vr07::calc_dim_bed_flux( double bed_flux_nodim ) {
...
double ClassSed_Transport__vr07::VANRIJN( double tau_b_nodim , double tau_crit_nodim, double diam_nodim ) {
...
double ClassSed_Transport__vr07::calc_bedload_transport_cap(double stream_depth) {  // runoff: mm * km^2/int
...
double ClassSed_Transport__vr07::calc_vr_transport_stage(double streamwidth) {
...
double ClassSed_Transport__vr07::calc_vr_ref_level(double streamdepth) {
  // reference level = roughness height if bedform height is not known
  return max(0.01*streamdepth, vr_roughness_height[hh] );
}

double ClassSed_Transport__vr07::calc_vr_ref_conc(double ref_level, double transport_stage) {
  return 0.015 * ( diam50[hh] / ref_level ) * ( pow(transport_stage,1.5) / pow(diam_nodim[hh],0.3) );   // van Rijn (84) Eq 38
}

double ClassSed_Transport__vr07::calc_vr_diam_susp(double transport_stage) {
  return diam50[hh] * ( 1 + 0.011 * (bed_material_sd - 1) * (transport_stage - 25) );   // van Rijn (84) Eq 39
}

double ClassSed_Transport__vr07::calc_vr_fallvel(double transport_stage) {
...
double ClassSed_Transport__vr07::calc_vr_shearvel(double streamdepth) {
  ...
  double streamdepth)   // m
{
  ...
  diam_nodim = calc_nodim_diam(diam50[hh]);
  double M_e = (streamvel - u_crit) / sqrt((s - 1) * g * diam50[hh]);

  return 0.012 *
         streamvel * diam50[hh] * pow(M_e, 2.4) * pow(diam_nodim, -0.6);
}

double ClassSed_Transport__vr07::calc_vr_load_transport_rect(double streamflow ) {
  ...
  mass_suspended[hh] = calc_suspended_transport_cap(flow_rate/(stream_depth*channel_width[hh]), channel_width[hh], stream_depth);
