...
ClassREWroute_storage* ClassREWroute_storage::klone(string name) const{
  return new ClassREWroute_storage(name);
}

void ClassREWroute_storage::decl(void) {

  Description = "'Handles the routing of surface and subsurface runoff from RBs (representative basins) including an upstream storage component.' \
                    'uses Muskingum,' \
                    'uses Clark.'";
  ...
  declvar("WS_channel_storage", TDim::NHRU, "no-flow storage in channel", "(m^3)", &channel_storage);
  declparam("WS_max_storage", TDim::NHRU, "[0.0]", "0.0","1.0E4.0", "no-flow max storage in channel", "(m^3)", &max_storage);
  declparam("WS_storage_decay", TDim::NHRU, "[0.0]", "0.0","1.0E4.0", "inflow lag delay", "(m^3/int)", &storage_decay);
...
void ClassREWroute_storage::init(void) {
  ...
  for(hh = 0; hh < nhru; ++hh) {
    ...
    channel_storage[hh] = 0.0;
  }
}

void ClassREWroute_storage::run(void) {
  ...
  for(long jj = 0; jj < inflowCnt; ++jj) { // HRUs not in sequence
    ...
    if (channel_storage[hh] < max_storage[hh]) {
      double extracted = fmin(max_storage[hh]-channel_storage[hh], inflow[hh]);
      channel_storage[hh] += extracted;
      inflow[hh] -= extracted;
    }
    channel_storage[hh] = fmax(0, channel_storage[hh]-storage_decay[hh]);
...
void ClassREWroute_storage::finish(bool good) {
