...
void ClassSoilX::decl(void) {
    ...
    declparam("lower_ssr_K", TDim::NHRU, "[0.0]", "0.", "100.0", "daily ssr drainage from lower soil layer (soil_moist - soil_rechr). Modified by thaw_lower.", "(mm/d)", &lower_ssr_K);

    declparam("Sd_ssr_K", TDim::NHRU, "[0.0]", "0.", "100.0", "daily ssr drainage factor from depression storage. Modified by thaw_all.", "(mm/d)", &Sd_ssr_K);
    ...
    for (hh = 0; chkStruct(); ++hh) {
        ...
        if (nstep == 0) {
            ...
            for (long ll = 0; ll < depths_size; ++ll) {
                ...
                if ( (NO_Freeze[hh] == 1) || (Zd_front_array[0][hh] == 0.0) )
