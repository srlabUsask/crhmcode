...
{
InstrumentLogger::instance()->log_instrument_log("<ClassK_Estimate::klone(string name) const@@@ClassK_Estimate.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassK_Estimate::decl(void)@@@ClassK_Estimate.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassK_Estimate::init(void)@@@ClassK_Estimate.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassK_Estimate::run(void)@@@ClassK_Estimate.cpp>");
    ...
    for (hh = 0; chkStruct (); ++ hh) {
        ...
        bitset < 6 > Inhibits (Inhibit_K_set [hh]);
        if (! Inhibits [0])
        {
        ...
        if (variation == VARIATION_1)
        {
            if (! (Dts_max [hh] == 0.0))
            {
                if (inhibit_evap [hh] == 1 && rho [hh] > 100)
                {
                    Ks_snow [hh] = (0.077 * sqr ((d_g [hh] / 1000)) * exp (- 7.8 * (rho [hh] / 1000))) * (den_water * acc_gravi / dyna_visc_water);
                    v_L_snow [hh] = Ks_snow [hh] * pow (Dts [hh] / Dts_max [hh], S_I [hh]) * sin (hru_GSL [hh] * DEGtoRAD);
                    v_L_organic [hh] = 0.0;
                }

                if (inhibit_evap [hh] == 0 || rho [hh] <= 100)
                {
...
{
InstrumentLogger::instance()->log_instrument_log("<ClassK_Estimate::finish(bool good)@@@ClassK_Estimate.cpp>");
InstrumentLogger::instance()->log_instrument_log("</ClassK_Estimate::finish(bool good)@@@ClassK_Estimate.cpp>");
}