...
{
InstrumentLogger::instance()->log_instrument_log("<Classbrushintcp::klone(string name) const@@@Classbrushintcp.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<Classbrushintcp::decl(void)@@@Classbrushintcp.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<Classbrushintcp::init(void)@@@Classbrushintcp.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<Classbrushintcp::run(void)@@@Classbrushintcp.cpp>");
    const double LATH = 2.838E6;
    const double Qstar = 120;
        ...
        if (Zref [hh] > 0.0)
        {
            ...
            else
            {
                ...
                if (hru_snow [hh] > 0.0)
                {
                    ...
                    Load [hh] = Load [hh] + I1;
                    Thru [hh] = Thru [hh] + (hru_snow [hh] - I1);
                }
                ...
                if (Load [hh] > 0.0)
                {
                    ...
                    double CanVent = Uh * exp (brushAtten [hh] * (Zcan [hh] / Ht [hh] - 1.0));
                    double Reyn = 2 * Mpr * CanVent / KinVisc;
                    double Nuss = 1.79 + 0.606 * sqrt (Reyn);
                    double Temp = hru_t [hh] + 273.15;
                    double Lamb = 0.00063 * Temp + 0.0673;
                    double Diff = 2.06E-5 * pow (Temp / 273.0, 1.75);
                    double Sigma2 = hru_rh [hh] / 100.0 - 1.0;
                    double Htran = 0.9 * M_PI * sqr (Mpr) * Qstar;
                    double SigmaZ = Sigma2 * (1.019 + 0.027 * log (Zcan [hh]));
                    if (SigmaZ > - 0.01)
                    {
                        SigmaZ = - 0.01;
                    }
                    ...
                    DmDt *= 24.0 * 3600.0 / Global :: Freq;
                    spherecoeff [hh] = 17.6 * pow (cumbrushsubl [hh] + 0.001, - 0.086);
                    brushsubl [hh] = DmDt * spherecoeff [hh] * 1000.0;
...
{
InstrumentLogger::instance()->log_instrument_log("<Classbrushintcp::finish(bool good)@@@Classbrushintcp.cpp>");
    for (hh = 0; chkStruct (); ++ hh) {
        if (Zref [hh] > 0.0)
        {
