...
{
InstrumentLogger::instance()->log_instrument_log("<Classglacier::klone(string name) const@@@Classglacier.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<Classglacier::decl(void)@@@Classglacier.cpp>");
    ...
    ObsCnt_Albedo = declreadobs ("glacier_Albedo_obs", TDim :: NHRU, "available Albedo observations", "()", & glacier_Albedo_obs, HRU_OBS_misc, true);
    ...
    declvar ("lagT_used", TDim :: NHRU, "lagged air temperature used for debris delays otherwise zero.", "(" + string (DEGREE_CELSIUS) + "C)", & lagT_used);
    declvar ("lagSW_used", TDim :: NHRU, "lagged incoming shortwave radiation used for debris delays otherwise zero.", "(W/m^2)", & lagSW_used);
    ...
    declgetvar ("*", "Qsisn_Var", "(W/m^2*int)", & Qsisn_Var);
...
{
InstrumentLogger::instance()->log_instrument_log("<Classglacier::init(void)@@@Classglacier.cpp>");
    ...
    for (hh = 0; hh < nhru; ++ hh) {
        ...
        for (long ll = 0; ll < nlay; ++ ll) {
            ...
            if (firn_h_array_init [ll] [hh] > 0.0)
            {
                ...
                firn_h_array [ll] [hh] = firn_h_array_init [ll] [hh] * 1000.0;
                firn_dens_array [ll] [hh] = firn_dens_array_init [ll] [hh];
                firn_init [hh] += firn_h_array [ll] [hh] * firn_dens_array [ll] [hh] / 1000.0;
                firn_depth [hh] += firn_h_array [ll] [hh];
                firn [hh] += firn_h_array [ll] [hh] * firn_dens_array [ll] [hh] / 1000.0;
            }
        ...
        if (Elev_Adj_glacier_surf [hh])
        {
            topo_elev [hh] = hru_elev [hh] - (firn_depth [hh] / 1000.0 + ice [hh] / ice_dens [hh]);
        }
...
{
InstrumentLogger::instance()->log_instrument_log("<Classglacier::run(void)@@@Classglacier.cpp>");
    long nstep;
    double rho;
    nstep = getstep ();
    if (nstep == 1)
    {
        for (hh = 0; hh < nhru; ++ hh) {
            glacier_h2o [hh] = SWE [hh] + firn [hh] + ice [hh];
            glacier_depth [hh] = (Common :: DepthofSnow (SWE [hh]) + firn_depth [hh]) / 1000.0 + ice [hh] / ice_dens [hh];
            glacier_Surf [hh] = glacier_depth [hh] + topo_elev [hh];
            Pa [hh] = 101.3f * pow ((293.0f - 0.0065f * glacier_Surf [hh]) / 293.0f, 5.26f);
        }
    }
    ...
    for (hh = 0; chkStruct (); ++ hh) {
        ...
        for (long nn = 0; nn < nfirn [hh]; ++ nn)
            firn [hh] += firn_h_array [nn] [hh] * firn_dens_array [nn] [hh] / 1000.0;
        ...
        else
        {
            ...
            else
            {
                if (SWE [hh] <= 0.0 || ice_Albedo [hh] > Albedo [hh])
                {
                    ...
                    {
                        glacier_Albedo [hh] = ice_Albedo [hh];
                    }
        ...
        if (use_debris [hh] && debris_h [hh] > 0.0)
        {
            ...
            if (fabs (debris_h [hh] - 0.1) < epsilon)
            {
                lagT [hh] = 0.0;
                lagSW [hh] = 1.0;
                TF [hh] = 0.066;
            ...
            if (fabs (debris_h [hh] - 0.2) < epsilon)
            {
                lagT [hh] = 3.0;
                lagSW [hh] = 3.0;
                TF [hh] = 0.0456;
            ...
            if (fabs (debris_h [hh] - 0.23) < epsilon)
            {
                lagT [hh] = 3.0;
                lagSW [hh] = 4.0;
                TF [hh] = 0.0438;
            ...
            if (fabs (debris_h [hh] - 0.3) < epsilon)
            {
                lagT [hh] = 5.0;
                lagSW [hh] = 5.0;
                TF [hh] = 0.0392;
            ...
            if (fabs (debris_h [hh] - 0.4) < epsilon)
            {
                lagT [hh] = 7.0;
                lagSW [hh] = 7.0;
                TF [hh] = 0.0334;
            ...
            if (fabs (debris_h [hh] - 0.5) < epsilon)
            {
                lagT [hh] = 10.0;
                lagSW [hh] = 11.0;
                TF [hh] = 0.0265;
            ...
            {
                Xdebris_melt_hrly [hh] = TF [hh] * lagT_used [hh] + SRF [hh] * (1 - glacier_Albedo [hh]) * lagSW_used [hh];
        ...
        if (variation == VARIATION_5)
        {
            ...
            Qh_kata [hh] = den_air [hh] * Cp_air * ((0.01 + K_kat [hh]) / 2) * ((hru_t [hh] + 273.15) - Ts_glacier);
            Qe_kata [hh] = (MOL_wt_ratio_h2o_to_air * den_air [hh] * Lv) / (Pa [hh] * 10.0) * ((0.01 + K_kat [hh]) / 2.0) * ((hru_ea [hh] - e_s) * 10.0);
            Cp_water [hh] = Cp_W0 - 2.55 * ((T_rain [hh] + 273.15) - FREEZE);
            Qp_kata [hh] = Cp_water [hh] * rho_h2o * net_rain_org [hh] / 1000.0 * ((T_rain [hh] + 273.15) - Ts_glacier) / (24.0 / Global :: Freq * 3600.0);
            Qmelt_kata [hh] = Qn_kata [hh] + Qh_kata [hh] + Qe_kata [hh] + Qp_kata [hh];
            Xmelt_int [hh] = Qmelt_kata [hh] / (rho_h2o * 316.825 * 1000.0) * 1000.0 * (24.0 / Global :: Freq * 3600.0);
            if (nstep == 1)
            {
        ...
        for (hh = 0; chkStruct (); ++ hh) {
            ...
            if (SWE [hh] <= 0.0 && (firn [hh] > 0.0 || ice [hh] > 0.0))
            {
                ...
                if (delay_melt [hh] <= julian ("now"))
                {
                    ...
                    Qmelt [hh] = Qmelt [hh] / 316.8 * 1000.0;
            ...
            if (firn [hh] > 0.0 || ice [hh] > 0.0)
            {
                if (nfirn [hh] > 0 && Qmelt [hh] > 0.0 && ! inhibit_firnmelt [hh])
                {
                    double h2o;
                    while (nfirn [hh] > 0 && Qmelt [hh] > 0.0) {
                        ...
                        else
                        {
                            ...
                            Qmelt [hh] -= h2o;
                            cumQmelt [hh] += h2o;
                            for (long nn = 0; nn < nfirn [hh] - 1; ++ nn) {
                ...
                if (ice [hh] > 0.0 && Qmelt [hh] > 0.0 && ! inhibit_icemelt [hh])
                {
                    if (Qmelt [hh] >= ice [hh])
                    {
                        icemelt [hh] = ice [hh];
                ...
                if (SWE_to_firn_Julian [hh] == Julian)
                {
                    ...
                    if (nfirn [hh] && firn_dens_array [nfirn [hh] - 1] [hh] >= 830.0)
                    {
                        firn_2ice [hh] = firn_dens_array [nfirn [hh] - 1] [hh] * firn_h_array [nfirn [hh] - 1] [hh] / 1000.0;
                        ice [hh] += firn_2ice [hh];
                    ...
                    if (SWE [hh] > 0.0)
                    {
                        ...
                        while (fabs (e) > 0.001 && h > 0.601 && ++ iter_max < 100);
                    ...
                    firn [hh] = firn_dens_array [0] [hh] * firn_h_array [0] [hh] / 1000.0;
                    if (nfirn [hh] > 0)
                    for (long nn = 1; nn < nfirn [hh]; ++ nn) {
                        ...
                        if (old_firn == 0.0)
                        {
                            ...
                            SS = SS + " firn_dens_array[nn][hh]: " + FloatToStrF (firn_dens_array [nn] [hh], TFloatFormat :: ffFixed, 10, 4) + " firn_h_array[nn][hh]: " + FloatToStrF (firn_h_array [nn] [hh], TFloatFormat :: ffFixed, 10, 4);
                            CRHMException TExcept (SS.c_str (), TExcept :: WARNING);
                            LogError (TExcept);
                        }

                        double h550 = 1000.0 / (RHOi * k0) * (log (550.0 / (RHOi - 550.0)) - log (rho / (RHOi - rho)));
                        if (Densification [hh] != 0)
                        {
                            ...
                            else
                            {
                                ...
                                firn_h_array [nn] [hh] = old_firn / firn_dens_array [nn] [hh] * 1000.0;
                                double t550 = 1.0 / (k0 * SWEAA [hh]) * (log ((RHOi - rho) / (RHOi - 550)));
...
{
InstrumentLogger::instance()->log_instrument_log("<Classglacier::finish(bool good)@@@Classglacier.cpp>");
...
double Classglacier :: DepthofSnow (double SWE)
{
InstrumentLogger::instance()->log_instrument_log("<Classglacier::DepthofSnow(double SWE)@@@Classglacier.cpp>");
    double rho;
    if (SWE > 1.0)
    {
        rho = 522.0 - 204700 / SWE * (1.0 - exp (- SWE / 673.0));

InstrumentLogger::instance()->log_instrument_log("</Classglacier::DepthofSnow(double SWE)@@@Classglacier.cpp>");
        return SWE / rho * 1000.0;
    }
