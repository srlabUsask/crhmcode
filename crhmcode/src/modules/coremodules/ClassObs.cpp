...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::klone(string name) const@@@ClassObs.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::decl(void)@@@ClassObs.cpp>");
    ...
    Global :: Warming_t = const_cast < double * > (this -> ClimChng_t);
    Global :: Warming_p = const_cast < double * > (this -> ClimChng_precip);
    ...
    decldiagparam ("tmax_allrain", TDim :: NHRU, "[4.0]", "-10", "10", "precip all rain if HRU air/ice bulb temperature above or equal to this value. Not used in Harder method.", "(" + string (DEGREE_CELSIUS) + ")", & tmax_allrain);
    decldiagparam ("tmax_allsnow", TDim :: NHRU, "[0.0]", "-10", "10", "precip all snow if HRU air/ice bulb temperature below this value. Not used in Harder method", "(" + string (DEGREE_CELSIUS) + ")", & tmax_allsnow);
...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::init(void)@@@ClassObs.cpp>");
    if (this -> GroupCnt < 2)
    {
...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::pre_run(void)@@@ClassObs.cpp>");
    Global :: HRU_OBS = const_cast < long ** > (HRU_OBS_Tables);
...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::run(void)@@@ClassObs.cpp>");
    double catchratio;
    ...
    for (hh = 0; chkStruct (); ++ hh) {
        ...
        if (nstep == 1 || Global :: Freq == 1)
        {
            ...
            case VARIATION_2:
                ...
                break;
            case VARIATION_1:
                hru_tmax [hh] = t_max [hh];
                hru_tmin [hh] = t_min [hh];
                hru_tmean [hh] = (hru_tmax [hh] + hru_tmin [hh]) / 2.0;
                hru_eamean [hh] = hru_ea [hh];
                hru_rhmean [hh] = hru_rh [hh];
                break;
            }
        }
        ...
        if (variation != VARIATION_2)
        {
            ...
            case 3:
                if (p != NULL)
                {
                    catchratio = 1.18 * exp (- 0.18 * hru_u [hh]);
                }

                if (ppt != NULL)
                {
                    catchratio = exp (- 0.2 * umean);
                }
            ...
            hru_rain [hh] = 0.0;
            hru_newsnow [hh] = 0;
            if (snow_rain_determination [hh] == 2)
            {
                Harder ();
                if (hru_snow [hh] > 0.0)
                {
            ...
            else
            {
                ...
                if (hru_p [hh] > 0.0)
                {
                    if (Use >= tmax_allrain [hh])
                    {
                        hru_rain [hh] = hru_p [hh];
                    }

                    else
                    if (Use <= tmax_allsnow [hh])
                    {
...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::Harder(void)@@@ClassObs.cpp>");
...
{
InstrumentLogger::instance()->log_instrument_log("<Classobs::finish(bool good)@@@ClassObs.cpp>");
