...
ClassQuickroute* ClassQuickroute::klone(string name) const{
  return new ClassQuickroute(name);
}

double ClassQuickroute::Function1(double *I, long hh) {
  return runDelay->ChangeLag(I, hh);
}

double ClassQuickroute::Function2(double *X, long hh) {
  return runDelay->ChangeStorage(X, hh);
}

void ClassQuickroute::decl(void) {

  Description = "'Handles the routing of surface runoff, subsurface runoff and HRU routing using the lag and route or Muskingum method for \"outflow\", \"runoutflow\", etc. Distributed flow.'";
  ...
  decllocal("Ktravel", TDim::NHRU, "travel time from Muskingum method", "(d)", &Ktravel);
  ...
  variation_set = VARIATION_0;   // Basic routing is with Muskingum
  ...
  variation_set = VARIATION_1;   // Basic routing is with Clark
...
void ClassQuickroute::init(void) {

  nhru = getdim(TDim::NHRU);

  if(soil_ssrDiv > 1){
    string S = "Quickroute:  \"soil_ssr\". Converting to mm/int";
    CRHMException TExcept(S.c_str(), TExcept::WARNING);
    LogError(TExcept);
  }

  if(soil_runoffDiv > 1){
    string S = "Quickroute:  \"soil_runoff\". Converting to mm/int";
    CRHMException TExcept(S.c_str(), TExcept::WARNING);
    LogError(TExcept);
  }

  if(soil_gwDiv > 1){
    string S = "Quickroute:  \"gw_flow\". Converting to mm/int";
    CRHMException TExcept(S.c_str(), TExcept::WARNING);
    LogError(TExcept);
  }
  ...
  if (variation == VARIATION_0) {
    ...
    for(hh = 0; hh < nhru; ++hh){
      ...
      if(Ktravel[hh] >= (Global::Interval/(2.0*route_X_M[hh]))){
        string S = string("'" + Name + " (Quickroute) Muskingum coefficient negative in HRU ").c_str() + to_string(hh+1);
        CRHMException TExcept(S.c_str(), TExcept::WARNING);
        LogError(TExcept);
      }
    ...
    hruDelay_mkg = new ClassMuskingum(inflow, outflow, Ktravel, route_X_M, Lag, nhru);

    for(hh = 0; hh < nhru; ++hh){
      if(Ktravel[hh] < (Global::Interval/(2.0*(1.0-route_X_M[hh])))){ //    if(hruDelay->c0[hh] < 0.0)
        hruDelay_mkg->c0[hh] = 0.0;
        hruDelay_mkg->c1[hh] = 1.0;
        hruDelay_mkg->c2[hh] = 0.0;
      }
    }
  }

  if(variation == VARIATION_1)
    hruDelay_clark = new ClassClark(inflow, outflow, Kstorage, Lag, nhru);
  ...
  for(hh = 0; hh < nhru; ++hh) {
    ...
    if(!OK){
        string SS = string("'" + Name + " (Quickroute)' the 'order' parameter does not have a unique value for each HRU");
...
// Do this for each HRU in 'ordering' order:
...
/**********************************************
 * Step 4: calculate ssrinflow, runinflow
 **********************************************/
...
/**********************************************
 * Step 5: apply ssr, runoff routing
 **********************************************/
...
/**********************************************
 * Step 8: apply 'general' routing
 **********************************************/

    if (hruDelay_mkg != NULL)
      hruDelay_mkg->DoMuskingum(hh);
    if (hruDelay_clark != NULL)
      hruDelay_clark->DoClark(hh);

/**********************************************
 * Step 9: partition 'general' outflow
 **********************************************/
...
// move to next HRU (given ordering)
...
void ClassQuickroute::finish(bool good) {
  ...
  for(hh = 0; chkStruct(); ++hh) {
    ...
    LogMessageA(hh, string("'" + Name + " (Quickroute)' cuminflow              (mm) (mm*km^2) (mm*basin): ").c_str(), cuminflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' cumoutflow             (mm) (mm*km^2) (mm*basin): ").c_str(), cumoutflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' cumoutflow_diverted    (mm) (mm*km^2) (mm*basin): ").c_str(), cumoutflow_diverted[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    if (hruDelay_mkg != NULL) 
      LogMessageA(hh, string("'" + Name + " (Quickroute)' hruDelay_in_storage    (mm) (mm*km^2) (mm*basin): ").c_str(), hruDelay_mkg->Left(hh)/hru_area[hh], hru_area[hh], basin_area[0]);

    LogMessageA(hh, string("'" + Name + " (Quickroute)' scaling_boost          (mm) (mm*km^2) (mm*basin): ").c_str(), cumscaling_boost[hh]/hru_area[hh], hru_area[hh], basin_area[0]);

    LogMessageA(hh, string("'" + Name + " (Quickroute)' ssrcuminflow           (mm) (mm*km^2) (mm*basin): ").c_str(), ssrcuminflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' ssrcumoutflow          (mm) (mm*km^2) (mm*basin): ").c_str(), ssrcumoutflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' ssrDelay_in_storage    (mm) (mm*km^2) (mm*basin): ").c_str(), ssrDelay->Left(hh)/hru_area[hh], hru_area[hh], basin_area[0]);

    LogMessageA(hh, string("'" + Name + " (Quickroute)' runoffcuminflow        (mm) (mm*km^2) (mm*basin): ").c_str(), runcuminflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' runoffcumoutflow       (mm) (mm*km^2) (mm*basin): ").c_str(), runcumoutflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' runDelay_in_storage    (mm) (mm*km^2) (mm*basin): ").c_str(), runDelay->Left(hh)/hru_area[hh], hru_area[hh], basin_area[0]);

    LogMessageA(hh, string("'" + Name + " (Quickroute)' gwcuminflow            (mm) (mm*km^2) (mm*basin): ").c_str(), gwcuminflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' gwcumoutflow           (mm) (mm*km^2) (mm*basin): ").c_str(), gwcumoutflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' gwcumdiverted          (mm) (mm*km^2) (mm*basin): ").c_str(), gwcumoutflow_diverted[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogMessageA(hh, string("'" + Name + " (Quickroute)' gwDelay_in_storage     (mm) (mm*km^2) (mm*basin): ").c_str(), gwDelay->Left(hh)/hru_area[hh], hru_area[hh], basin_area[0]);

    LogMessageA(hh, string("'" + Name + " (Quickroute)' cum_to_Sd              (mm) (mm*km^2) (mm*basin): ").c_str(), cum_to_Sd[hh], hru_area[hh], basin_area[0], " *** Added to this HRU Sd");
    LogMessageA(hh, string("'" + Name + " (Quickroute)' cum_to_soil_rechr      (mm) (mm*km^2) (mm*basin): ").c_str(), cum_to_soil_rechr[hh], hru_area[hh], basin_area[0], " *** Added to this HRU recharge");
    LogMessageA(hh, string("'" + Name + " (Quickroute)' HRU_cumbasinflow       (mm) (mm*km^2) (mm*basin): ").c_str(), HRU_cumbasinflow[hh]/hru_area[hh], hru_area[hh], basin_area[0]);
    LogDebug(" ");
  ...
  LogMessage(string("'" + Name + " (Quickroute)' cumulative surface & subsurface basinflow(m^3): ").c_str(), cumbasinflow[0]);
  LogMessage(string("'" + Name + " (Quickroute)' cumulative basingw  (m^3): ").c_str(), cumbasingw[0]);
  LogDebug(" ");

  LogMessage(string("'" + Name + " (Quickroute)' Allgwcuminflow (mm*basin): ").c_str(), Allgwcuminflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allgwcumoutflow (mm*basin): ").c_str(), Allgwcumoutflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allgwcumoutflowdiverted (mm*basin): ").c_str(), Allgwcumoutflowdiverted);
  LogDebug(" ");

  LogMessage(string("'" + Name + " (Quickroute)' Allcuminflow (mm*basin): ").c_str(), Allcuminflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allcumoutflow (mm*basin): ").c_str(), Allcumoutflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allcumoutflowdiverted (mm*basin): ").c_str(), Allcumoutflowdiverted);
  LogDebug(" ");

  LogMessage(string("'" + Name + " (Quickroute)' Allssrcuminflow (mm*basin): ").c_str(), Allssrcuminflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allssrcumoutflow (mm*basin): ").c_str(), Allssrcumoutflow);
  LogDebug(" ");

  LogMessage(string("'" + Name + " (Quickroute)' Allruncuminflow (mm*basin): ").c_str(), Allruncuminflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allruncumoutflow (mm*basin): ").c_str(), Allruncumoutflow);
  LogDebug(" ");

  LogMessage(string("'" + Name + " (Quickroute)' AllSdcuminflow (mm*basin): ").c_str(), AllSdcuminflow);
  LogMessage(string("'" + Name + " (Quickroute)' Allrechrcuminflow (mm*basin): ").c_str(), Allrechrcuminflow);
  LogDebug(" ");

  if (hruDelay_mkg != NULL)
    delete hruDelay_mkg;
  if (hruDelay_clark != NULL)
    delete hruDelay_clark;
  delete ssrDelay;
  delete runDelay;
  delete gwDelay;
}
