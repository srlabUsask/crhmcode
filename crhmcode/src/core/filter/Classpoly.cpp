...
{
InstrumentLogger::instance()->log_instrument_log("<Classpoly::Classpoly(ClassData * MyObs, string ToVar, string args, string argtypes): Classfilter (MyObs, ToVar, args, argtypes)@@@Classpoly.cpp>");
    readargs ();
InstrumentLogger::instance()->log_instrument_log("</Classpoly::Classpoly(ClassData * MyObs, string ToVar, string args, string argtypes): Classfilter (MyObs, ToVar, args, argtypes)@@@Classpoly.cpp>");
}
void Classpoly :: doFunc (long Obs, long Line)
{
InstrumentLogger::instance()->log_instrument_log("<Classpoly::doFunc(long Obs, long Line)@@@Classpoly.cpp>");
    if (! Error)
    {
        if (FirstTime)
        {
