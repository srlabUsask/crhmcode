...
{
InstrumentLogger::instance()->log_instrument_log("<Common::writefile(std :: string filepath, std :: string content)@@@Common.cpp>");
    std :: ofstream myfile;
...
{
InstrumentLogger::instance()->log_instrument_log("<Common::tokenizeString(std :: string css, std :: vector < std :: string > * stringVector)@@@Common.cpp>");
    stringVector -> clear ();
    std :: string inString;
    std :: string token;
    size_t tokenStart = 0;
